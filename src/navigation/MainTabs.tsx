...
function CenterButton({ onPress }) {
  ...
  const handlePressIn = () => {
    ...
    Animated.parallel([
      Animated.spring(scaleAnim,  { toValue: 0.85, tension: 300, friction: 6, useNativeDriver: false }),
      Animated.timing(rotateAnim, { toValue: 1, duration: 180, useNativeDriver: false }),
    ]).start();
  };
  ...
  const handlePressOut = () => {
    Animated.parallel([
      ...
      Animated.timing(rotateAnim, { toValue: 0, duration: 180, useNativeDriver: false }),
    ]).start();
    onPress?.();
  };
  ...
  const spin = rotateAnim.interpolate({
    inputRange: [0, 1], outputRange: ['0deg', '25deg'],
  });
...
/* ── TAB ITEM (tout en useNativeDriver: false) ── */
function TabItem({ tab, focused, onPress }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.timing(dotAnim, { toValue: focused ? 1 : 0, duration: 220, useNativeDriver: false }),
      Animated.timing(bgAnim,  { toValue: focused ? 1 : 0, duration: 220, useNativeDriver: false }),
    ]).start();
  }, [focused]);
  ...
  const handlePress = () => {
    ...
    Animated.sequence([
      Animated.timing(bounceAnim,  { toValue: 0.72, duration: 75, useNativeDriver: false }),
      Animated.spring(bounceAnim,  { toValue: 1.18, tension: 320, friction: 4, useNativeDriver: false }),
      Animated.spring(bounceAnim,  { toValue: 1,    tension: 200, friction: 8, useNativeDriver: false }),
    ]).start();
    onPress?.();
  };
...
function CustomTabBar({ state, descriptors, navigation }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.spring(barSlideAnim, { toValue: 0, tension: 55, friction: 12, delay: 400, useNativeDriver: false }),
      Animated.timing(barFadeAnim,  { toValue: 1, duration: 400, delay: 400, useNativeDriver: false }),
    ]).start();
  }, []);
        ...
        <View style={styles.tabBar}>
          <View style={styles.tabSide}>
            {LEFT_TABS.map((tab) => {
              ...
              return (
                <TabItem key={`left_${tab.name}`} tab={tab} focused={focused}
                  onPress={() => navigateTab(tab.name, state.routes[routeIndex].key, focused)} />
              );
            })}
          </View>

          <CenterButton onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy); setShowPicker(true); }} />
          ...
          <View style={styles.tabSide}>
            {RIGHT_TABS.map((tab) => {
              ...
              const focused = state.index === routeIndex;
              return (
                <TabItem key={`right_${tab.name}`} tab={tab} focused={focused}
                  onPress={() => navigateTab(tab.name, state.routes[routeIndex].key, focused)} />
              );
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  tabBar: {
    ...
    backgroundColor: 'rgba(10,12,18,0.97)', borderRadius: 28,
  ...
  centerBtn: {
    width: 62, height: 62, borderRadius: 31, backgroundColor: '#0E1015',
    borderWidth: 1.5, borderColor: T.gaming + '65', justifyContent: 'center', alignItems: 'center',
    shadowColor: T.gaming, shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.75, shadowRadius: 18, elevation: 18, overflow: 'hidden', position: 'relative',
  },
